...
// Luces
#define CUARTO  32  //Luz/Pin de Cuarto
#define ESTUDIO 33  //Luz/Pin de Estudio
#define LED     25  //Luz/Pin LED
#define BANO    26  //Luz/Pin de Baño
...
void setup() {
  ...
  int salidas[] = {LED_BUILTIN, TRANSISTOR, CUARTO, ESTUDIO, LED, BANO, GRADAS};
  ...
  escribir("Conf. ThingSpeak", 16, 40);
...
// Lee el código desde ThingSpeak y enciende o apaga las luces
void control_luces(){
  int cod = ThingSpeak.readIntField(CHANNEL_ID, 3, READ_API_KEY);
  if(ThingSpeak.getLastReadStatus() == 200 and cod != last_cod){
    last_cod = cod;
    bool residuo = 0;
...
void pantalla_principal(){
  ...
  if(estado) estados[5] = 1;

  uint8_t luces_on = 0;
  for(i=0; i<sizeof(luces)/sizeof(luces[0]); i++){
    if(estados[i]){
      luces_on++;
      OLED.fillCircle(i*8+52, 35 , 3, SSD1306_WHITE);
    }else OLED.drawCircle(i*8+52, 35, 3, SSD1306_WHITE);
  }
  OLED.setCursor(0, 32);
  OLED.printf("Luces:%u",luces_on);  
